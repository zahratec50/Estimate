...
//   useEffect(() => {
//     setPreview(selectedAnswer || null);
//     setFile(null);
...
//             <img src={preview} alt="preview" className="max-h-40 w-auto rounded shadow" />
...
//             <span className="text-gray-500 dark:text-gray-400">Upload your file</span>
...
import { useState, useEffect, useRef, DragEvent, useCallback } from "react";
import { QuestionItem, useAppStore, ImageOption } from "@/store/useAppStore";
import { MdFileUpload } from "react-icons/md";
...
export const FileQuestion = ({ question, selectedAnswer, setAnswer }: Props) => {
  const { isContinueAllowed, currentProjectId, isRegistered } = useAppStore();
  const [file, setFile] = useState<File | null>(null);
  const [selectedImages, setSelectedImages] = useState<ImageOption[]>([]);
  const inputRef = useRef<HTMLInputElement>(null);
  const [isDragging, setIsDragging] = useState(false);

  // بارگذاری پاسخ‌های قبلی از استور
  useEffect(() => {
    const storedAnswers: ImageOption[] = (() => {
      ...
      const answerEntry = allAnswers.find((a) => a.questionTitle === question.title);
      if (!answerEntry || !answerEntry.answer) return [];

      // اطمینان از اینکه answer یک آرایه از رشته‌هاست
      const selectedUrls = Array.isArray(answerEntry.answer)
        ? answerEntry.answer
      ...
      return selectedUrls
        .filter((url): url is string => typeof url === "string") // فقط رشته‌ها
        .map((url) => ({
          imageUrl: url,
  ...
  }, [question, currentProjectId, isRegistered]);
