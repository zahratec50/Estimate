...
const AnswerItem = ({ question, answer, onImageClick }: AnswerItemProps) => {
  ...
  return (
    <li className="border dark:border-none rounded-xl bg-secondary-50 dark:bg-secondary-800 shadow-sm">
      <button
        className="w-full flex justify-between items-center p-4 text-left"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className="w-full font-medium text-gray-800 dark:text-gray-100">
          {question}
        </span>
        <FiChevronDown
