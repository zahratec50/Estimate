...
export default function DashboardPage() {
  const projects = useAppStore((state) => state.projects);
  const completedQuizzes = useAppStore(state => state.completedQuizzes);
  ...
  return (
    <div className="px-4 sm:px-2 lg:px-8 py-6 space-y-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 w-full sm:w-auto">
          ...
          <Link
            ...
            className="border border-gray-300 px-4 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-secondary-700 dark:border-none text-center"
          >
            View History
          </Link>
        </div>
      ...
      <section>
        ...
        {sortedProjects.length === 0 ? (
          ...
          <motion.div
            ...
            initial="hidden"
            animate="show"
            className="w-full flex flex-col lg:flex-row flex-wrap gap-6"
          >
            <ProjectsGrid projects={sortedProjects} />
          </motion.div>
        )}
