...
export default function QuizNavigation({
  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      ...
      if (isFirstQuiz) {
        payload.preQuizAnswers = formatAnswersForServer(preQuizAnswers);
      } else {
        payload.mainQuizAnswers = project
          ? formatAnswersForServer(project.mainQuizAnswers)
          : formatAnswersForServer(mainQuizAnswersTemp);
        payload.currentProjectId = currentProjectId;
      }
        ...
        >
          ...
          <div className="relative">
            <button
              ...
              disabled={
                !quiz[currentStep - 1] ||
                !isContinueAllowed(quiz[currentStep - 1], isFirstQuiz) ||
                loading
              }
              className={clsx(
